...
class Funcs():
    ...
    def add_cliente(self):
        self.variaveis()
        self.conecta_bd()
        self.cursor.execute("""INSERT INTO cadastro(nome_cliente, telefone, cidade, sexo) VALUES (?, ?, ?, ?)""", (self.nome, self.tel, self.cidade, self.sexo))
        self.conn.commit()
        self.select_lista()
        self.limpa_clientes()
    ...
    def select_lista(self):
        ...
        self.conecta_bd()
        lista = self.cursor.execute(""" SELECT cod, nome_cliente, telefone, cidade, sexo from cadastro ORDER BY cod ASC; """)
    ...
    def altera_cliente(self):
        ...
        self.variaveis()
        self.conecta_bd()
        self.conn.execute("""UPDATE cadastro SET nome_cliente = ?, telefone = ?, cidade = ?, sexo = ? WHERE cod = ?; """, (self.nome, self.tel, self.cidade, self.sexo, self.codigo))
    ...
    def buscar_cliente(self):
        ...
        sexo = self.Tipvar.get()

        self.conecta_bd()

        # Precisa ver como otimizar essa coluna
        if sexo == '':
            self.cursor.execute(f"""SELECT cod, nome_cliente, telefone, cidade, sexo FROM cadastro WHERE nome_cliente LIKE '{nome}' AND cidade LIKE '{cidade}' AND telefone LIKE '{telefone}'""")
        else:
            self.cursor.execute(f"""SELECT cod, nome_cliente, telefone, cidade, sexo FROM cadastro WHERE nome_cliente LIKE '{nome}' AND cidade LIKE '{cidade}' AND telefone LIKE '{telefone}' AND sexo = '{sexo}' """)
...
class Applicativo(Funcs, Relatorio):
    ...
    def criando_widgets(self):
        ...
        # Drop down button e label sexo
        self.sexo = Label(self.frame_1, text='Sexo', bg='#BEBEBE', font=('Times New Romans', 9, BOLD), fg='#1F2214')

        self.sexo.place(relx=0.70, rely=0.35)

        self.Tipvar = StringVar()
        self.TipV = ('', 'M', 'F')
        self.Tipvar.set('')
        self.popup_sexo = OptionMenu(self.frame_1, self.Tipvar, *self.TipV)
        self.popup_sexo.config(bg='#107db2', fg='white', highlightbackground='#BEBEBE', font=('Times New Romans', 9, BOLD),activebackground='#6A5ACD', activeforeground='white')

        self.popup_sexo.place(relx=0.7, rely=0.47, relwidth=0.08)
        ...
        self.balao_sexo = tix.Balloon(self.frame_1)
        self.balao_sexo.bind_widget(
            self.popup_sexo,
            balloonmsg='Selecione o sexo')
    
        self.remover_seta_tix(self.balao_buscar, self.balao_limpar, self.balao_apagar, self.balao_novo, self.balao_alterar, self.balao_sexo)
